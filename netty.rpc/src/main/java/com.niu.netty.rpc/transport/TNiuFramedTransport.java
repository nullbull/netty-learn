package com.niu.netty.rpc.transport;
...
public class TNiuFramedTransport extends TTransport {
    ...
    public static class Factory extends TTransportFactory {
        ...
        public Factory() {
            maxLength_ = TNiuFramedTransport.DEFAULT_MAX_LENGTH;
        }
        ...
        public TTransport getTransport(TTransport base) {
            TNiuFramedTransport TNiuFramedTransport= new TNiuFramedTransport ( base, maxLength_ );
            TNiuFramedTransport.setIfUserProtocol ( user );
            TNiuFramedTransport.setRsa ( rsa );
            TNiuFramedTransport.setPrivateKey ( privateKey );
            TNiuFramedTransport.setPublicKey ( publicKey );
            return TNiuFramedTransport;
        }
    }

    public TNiuFramedTransport(byte version, byte heartbeat, byte rsa, byte zip) {
        this.version = version;
        this.heartbeat = heartbeat;
        this.rsa = rsa;
        this.zip = zip;
    ...
    public TNiuFramedTransport(TTransport transport, int maxLength, boolean ifUserProtocol) {
    ...
    public TNiuFramedTransport(TTransport transport, int maxLength) {
        this ( (byte) 1, (byte) 1, (byte) 0, (byte) 0 );
        transport_ = transport;
        maxLength_ = maxLength;
    }

    public TNiuFramedTransport(TTransport transport) {
        this ( (byte) 1, (byte) 1, (byte) 0, (byte) 0 );
        transport_ = transport;
        maxLength_ = TNiuFramedTransport.DEFAULT_MAX_LENGTH;
    }
        ...
        if (ifUserProtocol) {
            if (rsa == (byte) 1) {
                try {
                    ...
                    buf = NiuRsaUtil.encryptByPublicKey ( buf, publicKey );
                    len = buf.length;
                    thrift = len;
                    sign = NiuRsaUtil.sign ( buf, privateKey );
