 ...
 */
public class NiuBinaryProtocol extends TProtocol {
    ...
    public NiuBinaryProtocol(TTransport trans) {
        this(trans, false, true);
    }

    public NiuBinaryProtocol(TTransport trans, boolean readGenericMethod) {
        this(trans, false, true);
        setReadGenericMethod(readGenericMethod);
    }

    public NiuBinaryProtocol(TTransport trans, boolean strictRead, boolean strictWrite) {
        super(trans);
    ...
    public void writeMessageBegin(TMessage message) throws TException {
        ...
        if(!thriftNative){
            if(generic){
                this.writeByte ( NiuBinaryProtocol.first );
                this.writeByte ( NiuBinaryProtocol.second );
            } else {
                this.writeByte ( NiuBinaryProtocol.second );
                this.writeByte ( NiuBinaryProtocol.first );
            }

            NiuTrace niuTrace = TraceThreadContext.get ();
            if(niuTrace ==null){
                niuTrace = new NiuTrace();
            }
            niuTrace.write ( this );
        }
    ...
    public NiuTrace getNiuTrace() {
        return niuTrace;
    }

    public void setNiuTrace(NiuTrace niuTrace) {
        this.niuTrace = niuTrace;
    }
    ...
    public TMessage readMessageBegin() throws TException {
        ...
        if(!thriftNative){
            ...
            if ((first == NiuBinaryProtocol.first && second == NiuBinaryProtocol.second) ||
                    (first == NiuBinaryProtocol.second && second == NiuBinaryProtocol.first)
                    ) {
                NiuTrace niuTrace = new NiuTrace();
                niuTrace.read ( this );
                setNiuTrace(niuTrace);
                if(first== NiuBinaryProtocol.first && second== NiuBinaryProtocol.second){
                    setGeneric ( true );
    ...
    public static class Factory implements TProtocolFactory {
        ...
        public TProtocol getProtocol(TTransport trans) {
            NiuBinaryProtocol proto = new NiuBinaryProtocol(trans, this.strictRead_, this.strictWrite_);
            if (this.readLength_ != 0) {
